...
const RoomIncomeChart = () => {
    const [roomIncome,setRoomIncome]=useState({})
    ...
    const handleFetchRoomIncome=async()=>{
      const API_URL = import.meta.env.VITE_API_URL;
        try{
            const response=await axios.get(`${API_URL}/bookingHistory/getRoomIncome`)
            // console.log(response.data);
            // alert(response.data);
            setRoomIncome(response.data);
        }catch(error){
            // console.log(error);
            // alert(error);
            setShowAlert(true)
            setAlertType("failed")
            setAlertMessage("Fetching Room Income Failed");
        }
    }

    useEffect(()=>{
        handleFetchRoomIncome();
    },[])
  ...
  <CustomAlert type={alertType}  message={alertMessage} onClose={() => setShowAlert(false)}/>
        ...
        <div className='px-4'>
            <div className='mt-10' style={{paddingTop:"14vh"}}>
                <h1 className='text-2xl font-bold text-center' style={{padding:"50px 0px"}}>Room Income Details</h1>
                  {/* Charts */}
                  <div className='w-full flex flex-col gap-8 mt-12 px-4'>
                    ...
                    <div>
                      ...
                      <div className='w-full overflow-x-auto'>
                        <MyLineChart yearly={roomIncome?.lastThreeYearsTotalIncome} />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className='w-full overflow-x-auto'>
                        <MyBarChart monthly={roomIncome?.lastThreeMonthsTootalIncome}/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className='w-full overflow-x-auto'>
                        <div style={{ minWidth: '700px' }}> {/* ✅ Helps fit long axis */}
                          <CustomShapeBarChart  daily={roomIncome?.lastSeveDaysTotalIncome}/>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default RoomIncomeChart